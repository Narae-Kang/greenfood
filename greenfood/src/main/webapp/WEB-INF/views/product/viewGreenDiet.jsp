...
//              setTableHTML+='<tr><th></th><th>월</th><th>화</th><th>수</th><th>목</th><th>금</th><th></th></tr>';
//              for(var i=0;i<5;i++){ 
//                  setTableHTML+='<tr height="180">';
//                  for(var j=0;j<1;j++){
//                      setTableHTML+='   <c:forEach var="js" items="${listJS }">';
//                      setTableHTML+='   <fmt:formatDate value="${js.mdate}" pattern="yyyy-MM-dd" var="mdate" />';
//                      setTableHTML+='<td style="text-overflow:ellipsis;overflow:hidden;white-space:nowrap">';
// 											setTableHTML+='   <c:if test="${js.mdate == mdate}">';
//                      setTableHTML+='    <div class="cal-day"></div>';
//                      setTableHTML+='    <div class="cal-schedule">';
 //                     setTableHTML+='   <strong id="maindish-title"> ${js.maindish}</strong><br>';
 //                     setTableHTML+='    ${js.maindish}<br>';
      //                setTableHTML+='    ${js.rice}<br>';
  //                    setTableHTML+='    ${js.side1}<br>';
  //                    setTableHTML+='    ${js.side2}<br>';
    //                  setTableHTML+='    ${js.side3}<br>';
       //               setTableHTML+='    ${js.side_side}<br>';
     //                 setTableHTML+='    ${js.soup}<br>';
        //              setTableHTML+='    칼로리 : ${js.cal}kcal<br>';
        //              setTableHTML+='    ${mdate}<br>';
      //                setTableHTML+='    </div>';
      //                setTableHTML+='</td>';
        //              setTableHTML+='</c:if>'; 
       //               
...
//               }
    ...
    font-size: 0.9em;}
        ...
        .cal-schedule{    font-size: 0.85em; line-height: 20px; height: 115px;}
        ...
        function cal(value) {
                var calc = document.querySelector("#pnnumSelect");
						...
						var selected = calc.children[calc.selectedIndex];
						 if (selected.value == '그린다이어트 10일권') {
			                    calcTotalPrice = Math.round(10 * 100000 / 10);
			                } else if (selected.value == '그린다이어트 20일권') {
			                    calcTotalPrice = Math.round(11 * 181818 / 10);
			                } else if (selected.value == '그린다이어트 30일권') {
			                    calcTotalPrice = Math.round(12 * 250000 / 10);
			                }
											 var quantity = document.querySelector("#quantity"); 
			                if (selected.value == '그린다이어트 10일권') {
			                    calcTotalPrice2 = Math.round(10 * 100000 / 10) * quantity.value;
			                } else if (selected.value == '그린다이어트 20일권') {
			                    calcTotalPrice2 = Math.round(11 * 181818 / 10) *  quantity.value;
			                } else if (selected.value == '그린다이어트 30일권') {
			                    calcTotalPrice2 = Math.round(12 * 250000 / 10) * quantity.value;
			                }                    
				    ...
				    function init2(){
				      ...
				      //바로 주문하기
				        ...
				        e.preventDefault();
				     		let frm = document.forms['greenDiet'];
				     		//1)'원' 제거
 ...
 <div class="headerList">
    ...
    <div class="List-wrapper">
      <div class="container">
        <div class="wrapper">
          <div class="home-menu">
          <a href="<c:url value='/' />"><i class="fas fa-home"></i>HOME</a> > <a href="${pageContext.request.contextPath }/product/list">맛있는 한끼</a> > <a href="${pageContext.request.contextPath }/product/greenDiet/10">그린다이어트</a>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="List-wrapper2">
      <div class="itemList">
        ...
        <div class="main-image"><img src="<c:url value="/resources/img/diet.jpg"/>" width="550px" height="505px" alt="">
        </div>
        <form 		 id="greenDiet"
...
<%--               		<h1>day:${listJS.get(0).mdate.day}</h1> --%>
              		<c:forEach begin="2" end="${listDiet.get(0).mdate.day}">
              			<td></td>
              		</c:forEach>         
                  <c:forEach var="diet" items="${listDiet}" varStatus="status">
                  <fmt:formatDate value="${diet.mdate}" pattern="yyyy-MM-dd" var="mdate" />
                  <fmt:formatDate value="${diet.mdate}" pattern="d" var="day" />
                  <fmt:parseDate var="mdate" value="${diet.mdate}" pattern="yyyy-MM-dd" />
                  ...
                  <div class="cal-schedule">
								<c:if test="${diet.mdate == mdate}">                      
                  <strong id="maindish-title"> ${diet.maindish}</strong><br>
                  ${diet.side1}<br>
                  ${diet.soup}<br>
                      칼로리 : ${diet.cal}kcal<br>
                  </c:if> 
                  </div>
                  </td>
    ...
    <div class="consist-box-biggest">
      <div class="consist-box-bigger">
        <div class="consist-box">
          <div class="consist-image-box"><img src="<c:url value="/resources/img/diet-consist.jpg"/>" width="760px" height="460px" alt=""></div>
          <div class="consist-info">
            ...
            <div class="consist-of-menu">
              ...
              <div class="font-small"> (240g) </div>
            </div>
            <div class="consist-of-menu">
              <div class="font-strong">현미밥 </div>
              <div class="font-small"> 1종(120g) </div>
            </div>
      ...
      <div class="rice-wrapper">
          <div class="rice-img-inner">
            <img src="<c:url value="/resources/img/diet-main.jpg"/>" alt="">
            <div class="rice-info-center"> 맛과 건강 모두 잡은 저염식 메인요리입니다. (1인 기준 320g 내외)</div>
        </div>
      </div>
