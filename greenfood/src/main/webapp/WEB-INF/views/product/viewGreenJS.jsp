  ...
  <script type="text/javascript"> 
    function scroll_follow(id) {
      $(window).scroll(function ()  //스크롤이 움직일때마다 이벤트 발생
      {
        ...
        // if(position>100){
            $(id).stop().animate({ top: position + 125 + "px" },800); //해당 오브젝트 위치값 재설정
        // }else {
...
//           //calendar 날짜표시
    ...
    font-size: 0.9em;}
        ...
        .consist-title1-grandson{width: 1024px; text-align: left; height: 50px; line-height: 50px; font-size: 1.25em;}
          ...
          .consist-abstract-box{width: 100%; height: 190px;}
...
<body>
    <SCRIPT>
        ...
        function cal(value) {
                ...
                if (selected.value == '그린정식 10일권') {
                    calcTotalPrice = Math.round(1 * 1200000 / 10);
                } else if (selected.value == '그린정식 20일권') {
                    calcTotalPrice = Math.round(2 * 1200000 / 10);
                } else if (selected.value == '그린정식 30일권') {
                    calcTotalPrice = Math.round(3 * 1200000 / 10);
                }
                var quantity = document.querySelector("#quantity"); 
                if (selected.value == '그린정식 10일권') {
                    calcTotalPrice2 = Math.round(1 * 1200000 / 10) * quantity.value;
                } else if (selected.value == '그린정식 20일권') {
                    calcTotalPrice2 = Math.round(2 * 1200000 / 10) *  quantity.value;
                } else if (selected.value == '그린정식 30일권') {
                    calcTotalPrice2 = Math.round(3 * 1200000 / 10) * quantity.value;
                }                    
...
// 								var orgin = parseInt(val);
    ...
    function init2(){
      ...
      //바로 주문하기
        ...
        e.preventDefault();
     		let frm = document.forms['greenJs'];
     		//1)'원' 제거
 ...
 <div class="headerList">
    ...
    <div class="List-wrapper">
      <div class="container">
        <div class="wrapper">
          <div class="home-menu">
          <a href="<c:url value='/' />"><i class="fas fa-home"></i>HOME</a> > <a href="${pageContext.request.contextPath }/product/list">맛있는 한끼</a> > <a href="${pageContext.request.contextPath }/product/greenJs/1">그린정식</a>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="List-wrapper2">
      <div class="itemList">
        ...
        <div class="main-image"><img src="<c:url value="/resources/img/koreanfood.jpg"/>" width="550px" height="505px" alt="">
        </div>
        <form id="greenJs" 
...
<%--               		<h1>day:${listJS.get(0).mdate.day}</h1> --%>
              		<c:forEach begin="2" end="${listJS.get(0).mdate.day}">
              			<td></td>
              		</c:forEach>         
                  <c:forEach var="js" items="${listJS}" varStatus="status">
                  <fmt:formatDate value="${js.mdate}" pattern="yyyy-MM-dd" var="mdate" />
                  <fmt:formatDate value="${js.mdate}" pattern="d" var="day" />
                  <fmt:parseDate var="mdate" value="${js.mdate}" pattern="yyyy-MM-dd" />
                  ...
                  <div class="cal-schedule">
								<c:if test="${js.mdate == mdate}">                      
                  <strong id="maindish-title"> ${js.maindish}</strong><br>
                  ${js.rice}<br>
                  ${js.side2}<br>
                  ${js.side3}<br>
                  ${js.side_side}<br>
    ...
    <div class="consist-box-biggest">
      <div class="consist-box-bigger">
        <div class="consist-box">
          <div class="consist-image-box"><img src="<c:url value="/resources/img/consist-image.jpg"/>" width="760px" height="460px" alt=""></div>
          <div class="consist-info">
            <div class="consist-abstract-box"></div>
            <div class="consist-of-menu">
              <div class="font-strong">잡곡밥 </div>
              <div class="font-small"> (230g 내외) </div>
            </div>
            <div class="consist-of-menu">
              <div class="font-strong">국 </div>
              <div class="font-small"> (300 ~ 400g) </div>
            </div>
            <div class="consist-of-menu">
              <div class="font-strong">메인요리 </div>
              <div class="font-small"> (350~400g) </div>
            </div>
            <div class="consist-of-menu">
              <div class="font-strong">반찬 </div>
              <div class="font-small"> 4종(300~320g) </div>
            </div>
      ...
      <div class="rice-wrapper">
          <div class="rice-img-inner">
            <img src="<c:url value="/resources/img/rice-info.jpg"/>" alt="">
            <div class="rice-info-center"> 매일 다른 종류의 건강한 잡곡밥을 제공 합니다. (1인 기준 220~210g 내외)</div>
        </div>
      </div>
